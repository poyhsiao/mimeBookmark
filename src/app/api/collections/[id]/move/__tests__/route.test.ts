...
describe('POST /api/collections/[id]/move', () => {
  ...
  test('returns 401 when user is not authenticated', async () => {
    vi.mocked(mockSupabase.auth.getUser).mockResolvedValue({ data: { user: null } });

    const request = new NextRequest(new URL('/api/collections/col-1/move', 'http://localhost'), {
      method: 'POST',
      body: JSON.stringify({ parent_id: null }),
    });

    const response = await POST(request, { params: Promise.resolve({ id: 'col-1' }) });
  ...
  test('returns 400 when collection ID is missing', async () => {
    const request = new NextRequest(new URL('/api/collections//move', 'http://localhost'), {
      method: 'POST',
      body: JSON.stringify({ parent_id: null }),
    });
  ...
  test('returns 403 when user does not own collection', async () => {
    const mockFrom = {
      ...
      eq: vi.fn().mockReturnThis(),
      is: vi.fn().mockReturnThis(),
      single: vi.fn().mockResolvedValue({ data: { id: 'col-1', user_id: 'other-user' }, error: null }),
    };
    vi.mocked(mockSupabase.from).mockReturnValue(mockFrom as any);
  ...
  test('moves collection successfully', async () => {
    const mockFrom = {
      ...
      is: vi.fn().mockReturnThis(),
      single: vi.fn()
        .mockResolvedValueOnce({ data: { id: 'col-1', user_id: 'test-user-id' }, error: null })
        .mockResolvedValueOnce({ data: { id: 'parent-col', user_id: 'test-user-id' }, error: null }),
  ...
  test('moves collection to root', async () => {
    const mockFrom = {
      ...
      eq: vi.fn().mockReturnThis(),
      is: vi.fn().mockReturnThis(),
      single: vi.fn().mockResolvedValue({ data: { id: 'col-1', user_id: 'test-user-id' }, error: null }),
      update: vi.fn().mockReturnThis(),
    };
    mockFrom.update.mockResolvedValue({ error: null });
    vi.mocked(mockSupabase.from).mockReturnValue(mockFrom as any);
  ...
  test('returns 404 when parent is not owned by user', async () => {
    const mockFrom = {
      ...
      single: vi.fn()
        .mockResolvedValueOnce({ data: { id: 'col-1', user_id: 'test-user-id' }, error: null })
        .mockResolvedValueOnce({ data: { id: 'parent-col', user_id: 'other-user' }, error: null }),
    };
    vi.mocked(mockSupabase.from).mockReturnValue(mockFrom as any);
