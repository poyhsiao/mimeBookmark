...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!bookmark_id || !tag_ids || !Array.isArray(tag_ids) || tag_ids.length === 0) {
      return NextResponse.json({ error: 'bookmark_id and tag_ids array are required' }, { status: 400 });
    }
    ...
    const invalidTagIds = tag_ids.filter((id: string) => !validTagIds.includes(id));
    ...
    // 更新标签的使用计数
    const { error: rpcError } = await supabase.rpc('increment_tag_usage_count', { 
      tag_ids: validTagIds 
    });
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const tag_ids = searchParams.get('tag_ids')?.split(',').filter(Boolean) || [];

    if (!bookmark_id || tag_ids.length === 0) {
      return NextResponse.json({ error: 'bookmark_id and tag_ids are required' }, { status: 400 });
    }
    ...
    // 更新标签的使用计数
    const { error: rpcError } = await supabase.rpc('decrement_tag_usage_count', { 
      tag_ids: validTagIds 
    });
