...
vi.mock("@sentry/nextjs", () => ({
  init: vi.fn(),
  captureException: vi.fn(() => "mock-exception-id"),
  captureMessage: vi.fn(() => "mock-message-id"),
  setUser: vi.fn(),
  addBreadcrumb: vi.fn(),
  configureScope: vi.fn(
    (cb) => cb && cb({ setUser: vi.fn(), addBreadcrumb: vi.fn() }),
  ),
  getCurrentHub: vi.fn(() => ({ getClient: vi.fn(() => ({})) })),
}));

import {
  captureError,
  captureErrorMessage,
} from "@/lib/monitoring/capture-error";
import { captureMessage } from "@/lib/monitoring/capture-message";
import { setUser } from "@/lib/monitoring/set-user";
import {
  addBreadcrumb,
  createBreadcrumb,
} from "@/lib/monitoring/add-breadcrumb";
...
describe("capture-error", () => {
  ...
  describe("createBreadcrumb", () => {
    ...
    test("creates a breadcrumb with data", () => {
      const breadcrumb = createBreadcrumb("bookmark", "Bookmark deleted", {
        bookmarkId: "bm-999",
      });
...
describe("capture-message", () => {
  describe("captureMessage", () => {
    ...
    test("handles different message levels", () => {
      ...
      expect(
        captureMessage({ message: "Warning", level: "warning" }),
      ).toBeTruthy();
